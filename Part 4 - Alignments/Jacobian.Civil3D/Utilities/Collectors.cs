...
public static class Collectors
{
    ...
    /// <summary>
    /// Returns all alignments in the active Civil 3D document,
    /// wrapped as lightweight <see cref="Civil3DAlignment"/> objects.
    /// </summary>
    public static List<Civil3DAlignment> GetAlignments(Document doc)
    {
        var results = new List<Civil3DAlignment>();
        var civilDoc = CivilApplication.ActiveDocument;
        var db = doc.Database;
        ...
        using (Transaction tr = db.TransactionManager.StartTransaction())
        {
            ...
            foreach (ObjectId alignmentId in alignmentIds)
            {
                ...
                if(alignment != null)
                {
                    results.Add(new Civil3DAlignment(
                        alignment.Name,
                        alignment.ObjectId,
                        alignment.Length,
